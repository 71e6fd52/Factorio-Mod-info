...
| void read_name_fast(std::string name) | 以 Mod 名读取部分信息（如：rso-mod），速度更快 |
| void read_full_name(std::string name) | 以 Mod 完整名（"作者名/Mod 名"）读取（如：orzelek/rso-mod） |
| void read_url(std::string url) | 从 URL 读取 Mod 信息（不一定为 mods.factorio.com，只要 json 数据与 window.\_\_INITIAL\_STATE\_\_ 在同一行） |
### 读取信息
| 函数 | 读取内容 |
| ---- | -------- |
